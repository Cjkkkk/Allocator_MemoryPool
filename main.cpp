...
template<class T>
using AnoAllocator = y_mp_alloc<T>;
...
int main()
{
    ...
    // vector creation
    using IntVec = std::vector<int, STDAllocator <int>>;
    std::vector<IntVec, STDAllocator <IntVec>> vecints(TestSize);
    for (int i = 0; i < TestSize; i++)
        vecints[i].resize(dis(gen));

    using PointVec = std::vector<Point2D, STDAllocator <Point2D>>;
    std::vector<PointVec, STDAllocator <PointVec>> vecpts(TestSize);
    for (int i = 0; i < TestSize; i++)
